...
****************************************************************************************************************
 ...
 Compile with: gcc -o gCor5 gcor5.c -lm -lgsl -lgslcblas
 You must install:
...
-----------------------------
Version 1.5 (17 April 2014)
-----------------------------
-pack output graphs into a single window
-----------------------------
...
#include "gcor5_definitions.h"

int main(int argc, char **argv) {

  
  int i, j, k;
  printf("\n======= GCOR v1.5 - Automatic Gain Correction =======\n\tR. Lica, IFIN-HH, April 2014 \n\n");
  ...
  struct Data2Fit gnuplot_shData[regions];
  int onetime=0, cal_again=0;
  double refSpec[chNum], Spec[chNum], smoothSpec[high-low];
  double gnuplot_spec[4][chNum]; 
  int nData=0;
  double coeff[degree], gnuplot_coeff[degree], coeff_old[degree], chisq, norm;
  char answer;
  char title[3][200];
    ...
    if(irun[i]!=runref)
  {
    ...
    for (j=0; j<chNum; j++)
    {
      ...
      gnuplot_spec[0][j]=refSpec[j];
      gnuplot_spec[1][j]=Spec[j];
  ...
  if (answer!='a' && cal_again==0)  {
    
    for (j=0; j<nData; j++) gnuplot_shData[j] = shData[j];
    for (j=0; j<degree; j++) gnuplot_coeff[j] = coeff[j];
  ...
  for (j=0; j<chNum; j++)
    {
      refSpec[j]=rawSpec[irunref][idet*chNum+j];
      Spec[j]=rawSpec[i][idet*chNum+j];
      gnuplot_spec[2][j]=refSpec[j];
      gnuplot_spec[3][j]=Spec[j];
      
    }
  chiTest_cal=calibrate(refSpec, Spec, coeff);
  
 
  if (answer!='a' && cal_again==1) //disable the display in automatic mode
  {
    sprintf(title[0], "DET#%02d RUN#%04d", idet, irun[i]);
    sprintf(title[1], "Raw Spectra. ChiSq = %.2lf", chiTest);
    sprintf(title[2], "Recalibrated Spectra. ChiSq = %.2lf", chiTest_cal);
    gnuplot(gnuplotPipe, gnuplot_spec, gnuplot_shData, gnuplot_coeff, title); //degree = 2
    }
