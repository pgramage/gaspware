...
void   gnuplot(FILE * gnuplotPipe, double spec[][chNum], struct Data2Fit *shData,  double *coeff, char title[][200]) {
  
    
  char command[50][200];
  FILE * temp = fopen("gcor_fit-data.temp", "wt");
  FILE * out1 = fopen("gcor_spec1.temp", "wt");
  FILE * out2 = fopen("gcor_spec2.temp", "wt");
  int i;
  ...
  for (i=low; i<high-30; i++) {
    fprintf(out1, "%d\t%lf\t1\n", i, spec[0][i]);   // 1 sets the color
    fprintf(out2, "%d\t%lf\t1\n", i, spec[2][i]);
  }
  fprintf(out1, "\n");
  fprintf(out2, "\n");
  for (i=low; i<high-30; i++) {
    fprintf(out1, "%d\t%lf\t2\n", i, spec[1][i]);   // 2 sets the color
    fprintf(out2, "%d\t%lf\t2\n", i, spec[3][i]);
  }
  ...
  sprintf(command[0], "set multiplot layout 3,1 rowsfirst");
    // First graph
  sprintf(command[1], "set title \" %s \" ", title[0]);
  sprintf(command[2], "plot [0:1500][-%d:%d]'gcor_fit-data.temp' using 1:2:3 with yerrorbars, %lf + %lf*x", sweep, sweep, coeff[0], coeff[1]);
  sprintf(command[3], "set xlabel \"Channel\" \n set ylabel \"Shift\"");
  sprintf(command[4], "set key left top box 3");       //legend
    //Second graph
  sprintf(command[5], "set title \" %s \" ", title[1]);
  sprintf(command[6], "plot 'gcor_spec1.temp' u 1:2:3 with lines linecolor variable");
  sprintf(command[7], "set xlabel \"Channel\" \n set ylabel \"Counts\"");
  sprintf(command[8], "unset key");
    //Third graph
  sprintf(command[9], "set title \" %s \" ", title[2]);
  sprintf(command[10], "plot 'gcor_spec2.temp' u 1:2:3 with lines linecolor variable");
  sprintf(command[11], "set xlabel \"Channel\" \n set ylabel \"Counts\"");
  sprintf(command[12], "unset key");
  
  sprintf(command[13], "unset multiplot");
  
  for (i=0; i<14; i++) fprintf(gnuplotPipe, "%s \n", command[i]);
  fflush(gnuplotPipe); 
