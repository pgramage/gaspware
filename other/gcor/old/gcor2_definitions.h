...
Sensitivity=%lf\n\
Degree=%d\n\
LEFT | RIGHT\n", &chNum, &detNum, name, &runref, name, &runstop, &sweep, &sens, &degree);
...
double ratio(double *spec, int l, int n) { //checks if there is a significant change in the region; ration=max/min
  ...
  if (ratio > limit ) return limit/ratio;
  else return 0;
...
void gnuplot_spec(FILE * gnuplotPipe, double *spec1, double *spec2, char *title) {
  
  
  int i;
  FILE *out;
  char filename[200], labels[200], plot[200];
  sprintf(filename, "gcor_%s.temp", title);
  sprintf(labels, "set xlabel \"Channel\" \n set ylabel \"Counts\"");
  sprintf(plot, "plot 'gcor_%s.temp' u 1:2:3 with lines linecolor variable", title);
